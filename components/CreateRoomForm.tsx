...
const CreateRoomForm: React.FC<CreateRoomFormProps> = ({
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const buildingId = room
        ? room.buildingId
        : (formData.get("building") as string);
      ...
      if (!room) {
        ...
        const response = await fetch(
          `/api/building/${buildingId}/room/${room._id}/edit`,
          {
            method: "PATCH",
            body: JSON.stringify({ number, capacity }),
            headers: { "Content-Type": "application/json" },
    ...
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50 transition-transform duration-200 ease-out scale-100">
      <div className="bg-black p-6 rounded-xl w-96 shadow-lg border border-gray-700 relative">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-6">
          ...
          <button
            type="submit"
            className="bg-white text-black px-4 py-2 rounded-md font-mono"
            disabled={submitting}
          >
            {submitting
              ? room
