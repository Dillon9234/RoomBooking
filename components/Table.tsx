...
function GenericTable<T extends DataItem>({
  ...
  useEffect(() => {
    setCurrentitems(
      data.slice((currentPage - 1) * maxItems, currentPage * maxItems),
    );
  ...
  const handleAllCheckboxChange = () => {
    if (currentitems.length == selectedDeletingItems.length)
      setSelectedDeletingItems([]);
    else setSelectedDeletingItems(currentitems.map((cur) => cur._id));
  };
  ...
  return (
    <div className="w-full">
        ...
        <tbody>
          {currentitems.map((item, rowIndex) => (
            <tr key={item._id}>
              ...
              <td
                className={`px-4 py-2 w-[10em] border-[#575757] ${rowIndex !== currentitems.length - 1 ? "border-b" : ""}`}
              >
      ...
      <div className="flex justify-end p-5 gap-2">
        ...
        <button
          className="flex items-center justify-center px-3 py-1 border border-[#575757] rounded-lg"
        ...
        <button
          ...
          onClick={() =>
            setCurrentPage((prev) =>
              Math.min(prev + 1, Math.ceil(data.length / maxItems)),
            )
          }
        >
          Next
